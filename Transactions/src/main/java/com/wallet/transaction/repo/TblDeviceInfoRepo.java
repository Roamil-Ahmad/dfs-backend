...
import com.wallet.transaction.model.TblDeviceInfo;
...
public interface TblDeviceInfoRepo extends JpaRepository<TblDeviceInfo,Long> {
    ...
    /**
     * The device belonging to one app user.
     *
     * <p>Every other finder here reaches a device through an account or a mobile number, which
     * cannot tell one partner of a corporate agent from another - they all share the agent's
     * account. This one starts at the authenticated user, so another partner's device is never a
     * candidate.</p>
     *
    ...
    @Query(value = "SELECT D.* FROM TBL_APP_USER A \n" +
            ...
            "            WHERE T.ACCOUNT_NO=:accountNo AND C.IS_ACTIVE='Y' \n" +
            "            ORDER BY D.DEVICE_INFO_ID DESC FETCH FIRST 1 ROW ONLY",nativeQuery = true)
