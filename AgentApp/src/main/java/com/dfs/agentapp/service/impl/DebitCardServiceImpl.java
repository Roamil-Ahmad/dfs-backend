...
import com.dfs.agentapp.repo.LkpDebitCardTypeRepo;
import com.dfs.agentapp.repo.TblAccountRepo;
import com.dfs.agentapp.repo.TblDebitCardRequestRepo;
import com.dfs.agentapp.repo.TblDebitCardResponseRepo;
...
import com.dfs.agentapp.util.CustomException;
...
public class DebitCardServiceImpl extends HelperClass implements DebitCardService {
    ...
    /**
     * The agent an account belongs to.
     *
     * <p>This is where the agent app parts company with app. An account there hangs off
     * TBL_CUSTOMER, and app reads the cardholder identity from it. An agent account has no
     * customer at all - TBL_ACCOUNT.CUSTOMER_ID is null on every one of them - and carries its
     * identity on TBL_AGENT instead, so the same three reads have to come from there: AGENT_ID
     * where app sends CUSTOMER_ID, and TBL_AGENT.NID_NO where app sends the customer NID.</p>
     *
     * <p>Both are the same values in the card system's eyes: an id for the holder, and the
     * national id the relationship is keyed on. Copying app's version unchanged would have
     * thrown a NullPointerException on the first agent to request a card.</p>
     */
    private TblAgent agentOf(TblAccount tblAccount) {
        TblAgent tblAgent = tblAccount.getTblAgent();
        if (tblAgent == null) {
    ...
    public HashMap<String, Object> newCardRequest(NewRequest newRequest, Request request, BigDecimal userId)
        ...
        if (tblAccount != null) {
            ...
            if (tblDebitCardRequest != null) {
                ...
                tblDebitCardResponse.setCustomerId(String.valueOf(agentOf(tblAccount).getAgentId()));
    ...
    public HashMap<String, Object> changeCardPin(ChangePinRequest changePinRequest, Request request)
        ...
        if (!tblAccount.isEmpty()) {
            changePinRequest.setRelationshipNum(aeSencryption.decrypt(agentOf(tblAccount.get(0)).getNidNo()));
    ...
    public HashMap<String, Object> forgotCardPin(GeneratePinRequest forgotPinRequest, Request request)
        ...
        if (!tblAccount.isEmpty()) {
            String decryptedRelationshipNum = aeSencryption.decrypt(agentOf(tblAccount.get(0)).getNidNo());
    ...
    public HashMap<String, Object> cardLov() throws JsonProcessingException {
        ...
        // HelperClass keeps its ObjectMapper private in this service, so the json round trip goes
        // through its own helpers rather than the mapper directly. Same conversion either way.
        String json = convertObjecttoJson(response);
        CardResponse cardResponse = fromJson(json, CardResponse.class);
