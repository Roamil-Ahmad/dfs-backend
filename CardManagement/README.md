...
> Card wrapper service integrating the external card processor.
...
| Property | Value |
|----------|-------|
| **Service** | Card Management (`card-management`) |
| **Port** | `8005` |
| **Context-path** | `/cardwrapper` |
| **Swagger UI** | http://localhost:8005/cardwrapper/swagger-ui/index.html |
| **Database** | None (stateless service) |
| **Packaging** | WAR (runs standalone via `java -jar` in Docker; deploys to external Tomcat in prod) |

### 🔗 Dependencies
- **Calls →** External card processor API
- **Called by ←** App, BackOffice
...
docker compose logs -f card-management    # follow this service's logs
...
```
http://localhost:8005/cardwrapper/swagger-ui/index.html
```
